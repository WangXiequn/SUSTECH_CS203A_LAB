...
//        for (int i = 0; i <number ; i++) {
//            AVLTree.insertion(root,arr[i]);
//        }
//        for (int i = 0; i <number ; i++) {
//            AVLTree.deletion(root,arr[i]);
//            for (int j = 1; j <number-i ; j++) {
//                System.out.print(AVLTree.findKth(root,j)+" ");
//            }
//            System.out.println();
//        }
//    }
...
public class Main {
    ...
    public static void main(String[] args) throws IOException {
        ...
        int k = in.nextInt();
        int[] ms = new int[m];
        int[] ks = new int[m-k+1];
        int[] answerList = new int[m-k+1];
        for (int i = 0; i < m; i++) {
            ms[i] = in.nextInt();
        }
        ...
        AVLTree root = new AVLTree(ms[0]);
        for (int i = 1; i < k; i++) {
            AVLTree.insertion(root,ms[i]);
        }
        answerList[0]=AVLTree.findKth(root,ks[0]);
        for (int i = 0; i <m-k ; i++) {
            AVLTree.insertion(root,ms[i+k]);
            AVLTree.deletion(root,ms[i]);
            answerList[i+1]=AVLTree.findKth(root,ks[i+1]);
        }
        for (int i = 0; i < m-k+1; i++) {
